...
export async function scrapeAllianzDirectAvp(input: LiveScraperInput): Promise<LiveScraperResult> {
  ...
  try {
    ...
    await page.goto("https://www.allianzdirect.nl/aansprakelijkheidsverzekering/", {
    ...
    try {
      const ctaBtn = page.locator('a:has-text("Bereken"), button:has-text("Bereken"), a:has-text("Premie berekenen")').first();
